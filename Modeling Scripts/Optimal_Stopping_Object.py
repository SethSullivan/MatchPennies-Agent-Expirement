...
class Optimal_Decision_Time_Model():
    ...
    def prob_of_selecting_reaction(self): 
        output = np.zeros((self.num_blocks,len(self.timesteps[0,:])))
        for i in range(self.num_blocks):
            combined_uncertainty = np.sqrt(self.timing_uncertainty**2 + self.agent_stds[i]**2) # Prob of SELECTING only includes timing uncertainty
            ...
            output[i,:] = stats.norm.cdf(self.timesteps[i,:] - self.agent_means[i], 0 + self.decision_action_delay_mean,combined_uncertainty) # Probability that selecting at the current timestep will be greater than the agent mean (aka react)
        return output
    ...
    def calculate_mean_leave_target_time(self):
        ...
        self.prob_selecting_reaction_at_optimal = np.array(self.prob_selecting_reaction_at_optimal)
        self.prob_selecting_gamble_at_optimal = np.array(self.prob_selecting_gamble_at_optimal)
    ...
    def calculate_experiment_metrics(self):
        ...
        reach_time_gamble = self.optimal_gamble_leave_target_time + self.movement_time
        reach_time_reaction = self.optimal_reaction_leave_target_time + self.movement_time
        ...
        mean_diff = self.optimal_decision_time - self.agent_means
        std_diff = np.sqrt(self.timing_uncertainty**2 + self.agent_stds**2) 
        self.prob_react = 1 - stats.norm.cdf(self.decision_action_delay_mean, mean_diff,std_diff) # Probability that optimal decision time is after the agent goes (aka we react)  NEEDED TO DO THE TWO DISTRIBUTION DIFFERENCE THING
        self.prob_gamble = 1 - self.prob_react

        prob_indecision_if_react = 1 - stats.norm.cdf(1500,reach_time_reaction,self.reaction_plus_movement_uncertainty) # Probability that the reach time reaction is > 1500 
        prob_indecision_if_gamble = 1 - stats.norm.cdf(1500,reach_time_gamble,self.movement_uncertainty)
        self.prob_indecision = self.prob_react*prob_indecision_if_react + self.prob_gamble*prob_indecision_if_gamble
        ...
        self.prob_win = self.prob_react*prob_win_if_react + self.prob_gamble*prob_win_if_gamble
        self.perc_win = self.prob_win*100
        prob_incorrect_if_react = 0
        prob_incorrect_if_gamble = (1-prob_indecision_if_gamble)*0.5
        self.prob_incorrect = self.prob_react*prob_incorrect_if_react + self.prob_gamble*prob_incorrect_if_gamble
