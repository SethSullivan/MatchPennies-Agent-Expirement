...
class Optimal_Decision_Time_Simulation():
    def __init__(self,**kwargs):
        ...
        self.decision_to_action_delay_uncertainty = kwargs.get('decision_to_action_delay_uncertainty',10)
        ...
        # Get final results
        self.final_results = [x for x in self.simulation_results if x.ndim==2] # All results that have shape(26,26) and the results are not already a percent
        ...
        # Extend lists to include the gamble reaction stuff and mean times
        ...
        self.final_results_keys = ['Wins','Reaction Wins', 'Gamble Wins',
                                   ...
                                   'Number of Reaction Decisions', 'Number of Gamble Decisions', 
                                   'Perc Wins That Were Reaction','Perc Indecisions That Were Reaction','Perc Incorrects That Were Reaction',
                                   'Perc Wins That Were Gamble','Perc Indecisions That Were Gamble','Perc Incorrects That Were Gamble',
    ...
    def draw_times(self):
        ...
        if self.decision_to_action_delay_mean is not None:
            self.decision_to_action_delay = stats.norm.rvs(self.decision_to_action_delay_mean,self.decision_to_action_delay_uncertainty,size=(self.m,self.n,self.NUM_TRIALS))
        else:
    ...
    def simulation(self):
        ...
        self.player_wins, self.player_reaction_wins ,self.player_gamble_wins, \
        self.player_incorrects, self.player_reaction_incorrects, self.player_gamble_incorrects,\
        self.player_indecisions, self.player_reaction_indecisions, self.player_gamble_indecisions,\
        self.number_reaction_decisions, self.number_gamble_decisions,\
        self.perc_wins_that_were_reactions,self.perc_indecisions_that_were_reactions,self.perc_incorrects_that_were_reactions,\
        self.perc_wins_that_were_gambles,self.perc_indecisions_that_were_gambles,self.perc_incorrects_that_were_gambles,\
        self.player_decision_array, self.agent_decision_array, self.player_reach_times,\
        self.player_leave_target_times, self.player_decision_times, self.agent_decision_times = self.simulation_results
            
    def optimal(self,arr,percent=True):
        if arr.ndim == 2:
            result = np.full_like(self.agent_decision_means,np.nan, dtype=float) # dtype needs to be float for NAN to be in it
    ...
    def perc(self,x):
        if all([e.is_integer() for e in x]): # This checks to make sure it's not already a percent by checking if it's an integer
...
class Simulation_Experiment_Results(Optimal_Decision_Time_Simulation):
    ...
    '''
    def __init__(self, objects): #,reaction_time_mean,movement_time_mean,timing_uncertainty,movement_uncertainty,
                 #reaction_uncertainty):
        # super().__init__(reaction_time_mean,movement_time_mean,timing_uncertainty,movement_uncertainty,reaction_uncertainty)
        self.o1,self.o2 = objects
        self.reaction_time_mean = self.o1.reaction_time_mean
        self.movement_time_mean = self.o1.movement_time_mean
        self.timing_uncertainty,self.reaction_uncertainty,self.movement_uncertainty = self.o1.timing_uncertainty,self.o1.reaction_uncertainty,self.o1.movement_uncertainty,
        self.decision_to_action_delay_mean = self.o1.decision_to_action_delay_mean
        self.decision_to_action_delay_uncertainty = self.o1.decision_to_action_delay_uncertainty
        self.num_blocks = 6
...
class Grouped_Subjects():
    '''
    This class takes all the Simulation_Experiment_Results objects and puts them into one class
    so each metric is a (num_subject,condition) array for means/wins/etc.
    or a (num_subject,condition,trials) array for trial by trial stuff
    '''
    def __init__(self,objects,subnames):
        self.objects = objects
        self.subnames = subnames
        self.num_blocks = 6
        self.mean_metrics_across_subjects()
        self.grouped_mean_metrics()
        self.group_trial_metrics()
        
    def mean_metrics_across_subjects(self):
        self.num_results = len(self.objects[0].optimal_mean_results.keys()) # Get number of results from one of the objects
        self.all_subjects_sim_results = np.zeros((len(self.objects),self.num_results,self.num_blocks)) # Create 3d array to store all subjects 
        for i,sub in enumerate(self.objects): # Iterate trhoguh each subject
            j=-1
            for k,v in sub.optimal_mean_results.items(): # Iterate through the keys and values of the subject sim experiment results dict for each subject
                j+=1
                self.all_subjects_sim_results[i,j,:] = v # Put the 6 conditions for each subject for each metric
        # Take the mean across subjects
        self.mean_sim_results_across_subjects = np.mean(self.all_subjects_sim_results,axis=0)
        # Create new dictionary putting the results keys back in, with the new averaged simulation results
        self.mean_sim_results_across_subjects_dict = {}
        for metric,key in zip(self.mean_sim_results_across_subjects,self.objects[0].optimal_mean_results.keys()):
            self.mean_sim_results_across_subjects_dict.update({key:metric}) 
    def grouped_mean_metrics(self):
        num_results = len(self.objects[0].keys()) # Get number of results
        all_subjects_sim_results_dict = {}
        metrics = np.zeros((len(self.objects),self.num_blocks,num_results))
        j=-1
        for k,v in self.objects[0].items(): # Iterate through the keys and values of the subject sim experiment results dict for each subject
            j+=1
            for i,o in enumerate(self.objects): # Iterate trhoguh each subject
                metrics[i,:,j] = o[k] # Get the metric for that subject and store the 6 values in metrics array
            all_subjects_sim_results_dict.update({k:metrics[:,:,j]}) # Put the 6 conditions for each subject for each metric
