...
participant_iqr_movement_onset_time = iqr(group.movement_metrics.movement_onset_times("task"), axis=2, nan_policy="omit")
with open(constants.MODEL_INPUT_PATH / 'participant_iqr_movement_onset_time.pkl','wb') as f:
    dill.dump(participant_iqr_movement_onset_time,f)
...
#* Lookup table, not actually used
if False:
    ...
    def find_lookups(max_timing_sd = 100, timesteps = np.tile(np.arange(0.0, float(1800), 1), (2, 6, 1))):
        '''
        NOT USED, was going to do a lookup table fo reaction leave time and guess leave time after
        doing the truncation stuff but ended up calculating exactly each time
        '''
