...
class Optimal_Decision_Time_Model():
    def __init__(self, **kwargs):
        ...
        self.wtd_leave_target_time,self.wtd_reach_time = self.calc_wtd_mean_leave_and_reach_time()
        self.prob_making_wtd_reach_time = stats.norm.cdf(1500,self.wtd_reach_time,50)
        ...
        self.prob_making_reaction_based_on_agent = self.prob_making_based_on_agent()
        ...
        self.prob_win_total = self.prob_making_wtd_reach_time*(self.prob_success_reaction*self.prob_selecting_reaction + self.prob_success_gamble*self.prob_selecting_gamble)
        self.prob_incorrect_total = self.prob_making_wtd_reach_time*(self.prob_success_gamble*self.prob_selecting_gamble)
        self.prob_indecision_total = 1 - self.prob_making_wtd_reach_time
        ...
        self.metrics_name_dict = {'exp_reward': 'Expected Reward','exp_reward_gamble': 'Expected Reward Gamble','exp_reward_reaction':'Expected Reward Reaction',
                                  ...
                                  'prob_indecision_reaction':'Prob Indecision','prob_making_reaction_based_on_agent':'Prob Making Based on Agent'}
    ...
    def prob_making_when_reacting_or_gambling(self, react=False,gamble=False):
        ...
        if react:
            ...
            uncertainty = np.sqrt(self.movement_uncertainty**2) # Movement uncertainty and the decision action uncertainty, now we don't include timing uncertainty in this bc the decision to react or gamble depends on timing uncertainty and that's it
    ...
    def prob_making_based_on_agent(self):
        ...
        mean_sum = self.tiled_agent_means + self.reaction_plus_movement_time
        print(self.tiled_agent_means.shape)
    ...
    def calc_wtd_mean_leave_and_reach_time(self):
        self.tiled_agent_means = np.tile(self.agent_means,(2000,1)).T
        self.reaction_leave_time = (self.tiled_agent_means + self.reaction_plus_movement_time)
        self.gamble_leave_time  = (self.timesteps + self.decision_action_delay_mean)
        wtd_mean_leave_time =  self.reaction_leave_time*self.prob_selecting_reaction + self.gamble_leave_time*self.prob_selecting_gamble        
        mean_reach_time = wtd_mean_leave_time + self.movement_time
        
        self.prob_making_reaction_new = stats.norm.cdf(1500,self.reaction_leave_time,)
        return wtd_mean_leave_time,mean_reach_time
    ##########-------------------------------------###############################
    ...
    def calculate_experiment_metrics_alt(self):
        ...
        self.prob_win_optimal = self.prob_win_reaction_optimal + self.prob_win_gamble_optimal
        self.prob_incorrect_optimal = self.prob_incorrect_reaction_optimal + self.prob_incorrect_gamble_optimal
        self.prob_indecision_optimal = self.prob_indecision_reaction_optimal + self.prob_indecision_gamble_optimal
    ...
    def calculate_experiment_metrics(self):
        ...
        self.prob_indecision = self.prob_selecting_reaction_optimal*self.prob_indecision_if_react + \
        ...
        self.prob_win = self.prob_selecting_reaction_optimal*self.prob_win_if_react + self.prob_selecting_gamble_optimal*self.prob_win_if_gamble
        ...
        self.prob_incorrect = self.prob_selecting_reaction_optimal*self.prob_incorrect_if_react + self.prob_selecting_gamble_optimal*self.prob_incorrect_if_gamble
