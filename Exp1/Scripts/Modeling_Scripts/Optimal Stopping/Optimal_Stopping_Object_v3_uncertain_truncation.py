...
class Optimal_Decision_Time_Model():
    ...
    def prob_making_based_on_agent(self):
        ...
        cut_off_b_mean,cut_off_b_sd = self.timesteps,self.timing_uncertainty
        cut
        self.tiled_agent_means = np.tile(self.agent_means,(2000,1)).T
        self.tiled_agent_stds  = np.tile(self.agent_stds,(2000,1)).T
        self.a = (cut_off_a - self.tiled_agent_means)/self.tiled_agent_stds, (cut_off_b - self.tiled_agent_means) / self.tiled_agent_stds
        ...
        its = 100000
        rvs_agent = stats.norm.rvs(self.tiled_agent_means,self.tiled_agent_stds,its)
        rvs_timing = stats.norm.rvs(self.timesteps,self.timing_uncertainty,its)
        agent_mask = rvs_agent<=rvs_timing

        rvs_skewed_agent = rvs_agent*agent_mask
        rvs_skewed_agent[~agent_mask] = np.nan

        skew,mean,sd = stats.skewnorm.fit(rvs_agent[agent_mask])

        skewed_agent_pdf = stats.skewnorm.pdf(self.timesteps,skew,mean,sd)
