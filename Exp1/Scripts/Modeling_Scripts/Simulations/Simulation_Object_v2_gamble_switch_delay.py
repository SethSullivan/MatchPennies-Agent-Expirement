...
def simulation_runs(NUM_TRIALS, TRIAL_TIME, m, n, player_decision_times, agent_decision_times, 
                    decision_to_action_delay, switch_delay, reaction_times, movement_times, AGENT_MOVEMENT_TIME):
    ...
    for i in range(m):
        for j in range(n):
            for k in range(NUM_TRIALS):
                ...
                player_leave_target_times[i,j,k] = player_decision_times[i,j,k] + decision_to_action_delay[i,j,k] + switch_delay[i,j,k]
...
class Optimal_Decision_Time_Simulation():
    def __init__(self,**kwargs):
        ...
        self.switch_delay_mean = kwargs.get('switch_delay_mean',50)
        self.switch_delay_uncertainty = kwargs.get('switch_delay_uncertainty',10)
    ...
    def draw_times(self):
        ...
        if self.switch_delay_mean is not None:
            self.switch_delay = stats.norm.rvs(self.switch_delay_mean,self.switch_delay_uncertainty,size=(self.m,self.n,self.NUM_TRIALS))
        else:
            self.switch_delay = np.zeros((self.m,self.n,self.NUM_TRIALS))
    ...
    def simulation(self):
        self.simulation_results = simulation_runs(self.NUM_TRIALS,self.TRIAL_TIME, self.m, self.n, player_decision_times=self.player_decision_times, 
                                                  decision_to_action_delay=self.decision_to_action_delay, switch_delay=self.switch_delay,
