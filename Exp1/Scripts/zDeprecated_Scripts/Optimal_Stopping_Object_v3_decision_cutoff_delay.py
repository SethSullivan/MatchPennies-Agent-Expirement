...
3/6/23 - Delay on decision time
- In order to react, I will have to make my decision 50ms after the agent
   - I don't know how this makes sense but when I fit reaction and gamble decisions, the reaction and gamble decision times don't add up
   - 
...
class Optimal_Decision_Time_Model():
    ...
    def run_model(self):
        ...
        if True:
            ...
            self.prob_making_gamble = self.prob_making_gamble()
    ...
    def calculate_gamble_reaction_probs_from_expected_reward(self):
        ...
        if self.unknown_gamble_uncertainty_on:
            for i in range(6):
                ...
                self.prob_selecting_reaction_optimal[i] = 1 - stats.norm.cdf(0,diff,combined_uncertainty) # I've determined that the decision time just needs to be after, doesn't necessarily need to be after some decision action delay
    ...
    def mseloss(self,decision_time):
        ...
        metric_loss = np.array([win_diff,indecision_diff,incorrect_diff,leave_target_time_diff,
                                perc_reactions_diff,perc_gambles_diff])
        return metric_loss
