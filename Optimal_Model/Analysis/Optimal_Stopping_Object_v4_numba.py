...
spec = OrderedDict()
spec['arr_1d'] = nb.float64[:]
spec['arr_2d'] = nb.float64[:,:]
spec['arr_3d'] = nb.float64[:,:,:]
spec['num_float'] = nb.float64
...
class Optimal_Decision_Time_Model:
    def __init__(self, **kwargs):
        ...
        self.nsteps      = 1
        self.timesteps   = kwargs.get('timesteps',np.tile(np.arange(0,1600,self.nsteps),(self.num_blocks,1)))
        ...
        if True:
            ...
            self.condition_one = np.tile(self.reward_matrix[0],(self.timesteps.shape[-1],1))
            self.condition_two = np.tile(self.reward_matrix[1],(self.timesteps.shape[-1],1))
            self.condition_three = np.tile(self.reward_matrix[2],(self.timesteps.shape[-1],1))
            self.condition_four = np.tile(self.reward_matrix[3],(self.timesteps.shape[-1],1))
    ...
    def plot_optimals(self,metrics,num_plots = None ,dpi=125):
        ...
        for i in range(num_plots):
            ...
            ax.set_xlim(0,1500)
            ax.set_xticks(np.arange(0,self.timesteps.shape[-1],300))
    ...
    def prob_agent_go(self):
        ...
        for i in range(self.num_blocks):
            output[i,:] = norm.cdf(self.timesteps[i,:],self.agent_means[i],self.agent_sds[i])        
        return output
    ...
    def prob_of_selecting_reaction(self): 
        ...
        if self.known_gamble_uncertainty_on:
            ...
            if isinstance(self.known_gamble_uncertainty,np.ndarray):
                ...
                combined_uncertainty = np.tile(combined_uncertainty,(self.timesteps.shape[-1],1)).T
        ...
        for i in range(self.num_blocks):   
            ...
            output[i,:] = 1 - vector_cdf(cutoff,diff,sd) # I've determined that the decision time just needs to be after, doesn't necessarily need to be after some decision action delay
        return output
    ...
    def prob_making_for_reaction(self):
        ...
        x = np.full_like(self.cutoff_agent_reaction_mean,1500)
        ...
        prob_make_reaction = vector_cdf(x,mu,np.sqrt(self.cutoff_agent_reaction_sd**2 + self.reaction_plus_movement_uncertainty**2))
    ...
    def prob_making_for_gamble(self):
        ...
        output = np.zeros((self.num_blocks,len(self.timesteps[0,:])))
        # for i in range(self.num_blocks): 
        #     output[i,:] = norm.cdf(1500,self.gamble_reach_time_mean[i,:],self.gamble_reach_time_uncertainty[i])
        x = np.full_like(self.gamble_reach_time_mean,1500)
        mu = self.gamble_reach_time_mean
        sd = np.tile(self.gamble_reach_time_uncertainty,(self.timesteps.shape[-1],1)).T
        output = vector_cdf(x,mu,sd)
        return output
    ...
    def calculate_prob_of_selecting_reaction_or_gamble(self):
        ...
        #     self.prob_selecting_reaction_optimal_calc[i] = 1 - norm.cdf(self.weird_reaction_gamble_cutoff,diff,combined_uncertainty) # I've determined that the decision time just needs to be after, doesn't necessarily need to be after some decision action delay
        combined_uncertainty = np.sqrt(self.timing_uncertainty**2 + self.agent_sds**2)
        diff = self.optimal_index - self.agent_means
        x = np.full_like(diff,self.weird_reaction_gamble_cutoff)
        sd = combined_uncertainty
        self.prob_selecting_reaction_optimal_calc = 1 - vector_cdf(x,diff,sd) #
    ...
    def prob_making_for_reaction_and_gamble_optimal_calc(self):
        ...
        x = np.full_like(self.reaction_reach_time_optimal_calc,1500)
        
        output1 = vector_cdf(x,self.reaction_reach_time_optimal_calc,self.reaction_reach_time_uncertainty)
        ...
        output2 = vector_cdf(x,self.gamble_reach_time_mean,self.gamble_reach_time_uncertainty)
    ...
    def fit_model_to_data(self,data):
        ...
        '''
        self.tune_data      = data
        self.tune_timesteps = np.arange(900,1600,1)
