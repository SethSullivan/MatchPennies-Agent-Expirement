...
def generate_dataframe(group, EXPERIMENT = 'Exp1',DROP_SUBJECT_NUM = 13):
    ...
    df_metrics = pd.DataFrame(
        np.array([median_movement_onset_time,median_movement_time,wins,indecisions,incorrects,correct_decisions,wins_when_both_decide,
                                        gamble_movement_onset_time,movement_onset_time_sd,q1_median_movement_onset_time,q3_median_movement_onset_time,
                                        reaction_decisions,gamble_decisions,points]).T,
        columns=['Median_Movement_Onset_Time','Median_Movement_Time','Wins','Indecisions','Incorrects','Correct_Decisions','Wins_When_Both_Decide',
                'Median_Gamble_Movement_Onset_Time','SD_Movement_Onset_Time','Q1_Movement_Onset_Time','Q3_Movement_onset_time','Reaction_Decisions','Gamble_Decisions','Points']
    )
...
class Anova():
    def __init__(self, dependent_variable:str, inputs: Inputs, anova_type='rm_anova',):
        self.inputs = inputs
    ...
    def anova(self,):
        ...
        assert (self.inputs.df[self.inputs.FACTOR1].str.contains('1000').any() 
        ...
        if self.anova_type == 'rm_anova':
            ans = pg.rm_anova(data=self.inputs.df, dv=self.dependent_variable, within=[self.inputs.FACTOR1,self.inputs.FACTOR2], 
...
class Bootstrap():
    def __init__(self, inputs: Inputs, anova_obj: Anova,
                 change_m = None, alternative='two-sided',test='mean',no_collapse=False,
                 **kwargs):
        self.inputs = inputs
        ...
        self.collapse_factor = None # Defined in run_boostrap
    ...
    def run_bootstrap(self):
        def _collapse_across(arr,combos):
            ans = []
            if len(combos[0])==2:
                for combo in combos:
                    a = int(combo[0])
        ...
        if self.anova['p-GG-corr'][2] <0.05 or self.no_collapse:
            ...
            pval_dict,cles_dict = self.pairwise_bootstrap(self.metric,collapsed = False)
        ...
        #* Collapse
            ...
            #* Factor 1 collapse
            f1_collapse_metric = _collapse_across(self.metric,self.inputs.f1_collapse_combos) 
            f1_collapse_pvals_dict,f1_collapse_cles_dict = self.pairwise_bootstrap(f1_collapse_metric,collapsed=True,condition_nums=self.inputs.f1_condition_nums)
            
            #* Factor 2 collapse
            f2_collapse_metric = _collapse_across(self.metric,self.inputs.f2_collapse_combos) 
            f2_collapse_pvals_dict,f2_collapse_cles_dict = self.pairwise_bootstrap(f2_collapse_metric,collapsed=False,condition_nums=self.inputs.f2_condition_nums)
    ...
    def pairwise_bootstrap(self,data,collapsed,condition_nums=None,
                           **kwargs):
